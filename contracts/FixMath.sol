...
contract FixMath {
   ...
   */
  function toStrUint(uint256 convertValue)
    public
    view
    ...
    returns (int256 result, uint256 multiplier)
  {
    ...
    assembly {
      ...
      default {
        // if the length for the pointer is immediately 0, it won't get into the loop, so we have to calc a single value
        if eq(strLen, 0x00) {
          result := sub(result, sub(and(shr(0xf8, mload(ptr)), 0xff), 0x30))
        }
        ...
        for {
          ...
          case 0x2e {
            ...
            if eq(strLen, 0x00) {
              value := and(shr(0xf8, mload(ptr)), 0xff)
              result := sub(result, mul(multiplierForNumber, sub(value, 0x30)))
            }
          }
          // otherwise
          default {
   ...
   * - max uint value - type(int256).max
   */
  function toStrInt(int256 convertValue)
    public
    view
    returns (string memory result)
  {
