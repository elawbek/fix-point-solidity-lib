...
library FixPointLib {
     ...
     * @dev
     * - max length of the returned string - 79 symbols (include dot)
     * - max uint value - type(uint256).max
     * - dot value MUST be less than 78
     */
    function toStrUint(uint256 convertValue, uint256 dot)
        internal
        pure
     ...
     */
    function toUint(string memory str, uint256 dot)
        internal
        pure
        returns (uint256 result)
    {
        assembly {
            ...
            // revert if interger part of number > (78 - dotPosition)
            ...
            if and(gt(dot, counter), dotExist) {
    ...
    /**
     * @notice convert string to uint for mul/div, considering the dot
     *
     * @dev
     * - revert if the fractional part of the number is greater in length than the maximum
     */
    function toUintMulDiv(string memory str, uint256 dot)
        internal
        pure
            ...
            for {
                ...
                case 0x2e {
                    ...
                    if eq(strLen, 0x00) {
                        ...
                        result := add(
                            result,
                            mul(multiplierForNumber, sub(value, 0x30))
                        )
                    }
                }
                // otherwise
     ...
     */
    function toInt(string memory str, uint256 dot)
        internal
        pure
    ...
    /**
     * @notice convert string to int for mul/div, considering the dot
     *
     * @dev
     * - revert if the fractional part of the number is greater in length than the maximum
     */
    function toIntMulDiv(string memory str, uint256 dot)
        internal
        pure
